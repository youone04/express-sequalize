...
export const UpdateUser = async(req, res) => {
    try{
       ...
       if(konfirmasiNewPassword !== newPassword) return res.send({status: 400 , message: "Password tidak cocok"})

       const salt = await bcrypt.genSalt();
       const hashPassword  = await bcrypt.hash(newPassword.toString() , salt);
        await dbs.user.update({nama , password:hashPassword} , {
            where : {
                email: email
            }
        })
...
export const DeleteUser = async(req, res) => {
    try{
      ...
      if(!passwordUpdate) return res.send({status: 400 , message: "User Tidak Ada"})
       const match = await bcrypt.compare(password , passwordUpdate.password);
       if(!match)  return res.send({status: 400 , message: "Password salah"})
